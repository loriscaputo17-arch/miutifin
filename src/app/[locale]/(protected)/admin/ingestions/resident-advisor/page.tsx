...
export default function AdminIngestionsPage() {
  ...
  async function runIngest() {
    ...
    try {
      // 🔑 forza AMP (RA safe)
      const ampUrl = url.includes("?amp") ? url : `${url}?amp`;

      // 1️⃣ run ingest
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/ingestions/resident_advisor`,
        {
          method: "POST",
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-white/40 text-sm mt-1">
          Run manual ingestions from external sources
        </p>
      </div>
      ...
      <div className="bg-[#0c0c0c] border border-white/5 rounded-xl p-6 space-y-4 max-w-xl">
        ...
        <div className="space-y-1">
          <label className="text-xs text-white/40">Event URL</label>
          <input
            value={url}
            onChange={e => setUrl(e.target.value)}
            className="input"
            placeholder="https://it.ra.co/events/2369486"
          />
          <p className="text-xs text-white/30">
            AMP will be added automatically (?amp)
          </p>
        </div>
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-white/40 mt-1 space-y-0.5">
                      {e.start_at && (
                        <p>{new Date(e.start_at).toLocaleString("it-IT")}</p>
                      )}
                      {e.venue_name && <p>{e.venue_name}</p>}
                    </div>
                  </div>
