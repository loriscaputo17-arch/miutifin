...
export default function AdminIngestionsXceedPage() {
  ...
  async function runIngest() {
    ...
    try {
      // 1️⃣ run ingest (XCEED)
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/ingestions/xceed`,
        {
          method: "POST",
      ...
      if (!res.ok) {
        throw new Error("Xceed ingest failed");
      }
  ...
  return (
    <div className="space-y-8">
      ...
      {/* HEADER */}
      <div>
        <h1 className="text-2xl font-semibold">Xceed ingestion</h1>
        <p className="text-white/40 text-sm mt-1">
          Manually ingest events from Xceed
        </p>
      </div>
      ...
      <div className="bg-[#0c0c0c] border border-white/5 rounded-xl p-6 space-y-4 max-w-xl">
        ...
        {/* URL */}
        <div className="space-y-1">
          <label className="text-xs text-white/40">Xceed event URL</label>
          <input
            value={url}
            onChange={e => setUrl(e.target.value)}
            className="input"
            placeholder="https://xceed.me/it/milano/event/..."
          />
        </div>
        ...
        <button
          ...
          className="btn"
        >
          {loading ? "Running Xceed ingest…" : "Run Xceed ingest"}
        </button>
      </div>
